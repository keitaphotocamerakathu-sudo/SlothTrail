(() => {
  ...
  const state = { user:null, events:[], eventId:null, checkpoints:[], teams:[], fields:[], realtime:null };
  ...
  async function claimAdmin(){
    ...
    if(error){UI.toast(error.message.includes('ADMIN_ALREADY_EXISTS')?'มีผู้ดูแลระบบคนแรกแล้ว':error.message,'error');return;}
  ...
  async function saveEvent(e){
    ...
    let res;
    if(id) res=await db.from('trail_events').update(payload).eq('id',id).select().single();
    else res=await db.from('trail_events').insert({...payload,created_by:state.user.id}).select().single();
    if(res.error){UI.toast(res.error.message,'error');return;}
    if(!id){await db.from('trail_event_settings').insert({event_id:res.data.id});state.eventId=res.data.id;}
  ...
  function openCheckpoint(cp=null){
    if(!state.eventId){UI.toast('กรุณาสร้าง/เลือก Event ก่อน','error');return;}
    $('checkpointModalTitle').textContent=cp?'แก้ไข RC':'เพิ่ม RC'; $('checkpointId').value=cp?.id||''; $('checkpointNo').value=cp?.checkpoint_no||state.checkpoints.length+1; $('checkpointName').value=cp?.name||''; $('checkpointClue').value=cp?.clue||''; $('checkpointDistanceText').value=cp?.distance_text||''; $('checkpointPoints').value=cp?.points??50; $('checkpointLat').value=cp?.latitude??''; $('checkpointLng').value=cp?.longitude??''; $('checkpointFinish').checked=!!cp?.is_finish; $('checkpointModal').classList.add('show');
  }
  async function saveCheckpoint(e){
    e.preventDefault(); const id=$('checkpointId').value;
    const payload={event_id:state.eventId,checkpoint_no:Number($('checkpointNo').value),name:$('checkpointName').value.trim(),clue:$('checkpointClue').value.trim()||null,distance_text:$('checkpointDistanceText').value.trim()||null,points:Number($('checkpointPoints').value)||0,latitude:$('checkpointLat').value?Number($('checkpointLat').value):null,longitude:$('checkpointLng').value?Number($('checkpointLng').value):null,is_finish:$('checkpointFinish').checked,sort_order:Number($('checkpointNo').value)};
  ...
  async function loadTeams(){
    ...
    $('teamsTable').innerHTML=`<div class="table-wrap"><table class="table"><thead><tr><th>ทีม</th><th>สถานะ</th><th>ชำระเงิน</th><th>เวลาออกตัว</th><th>คะแนน</th><th>จัดการ</th></tr></thead><tbody>${state.teams.map(t=>{const slot=t.trail_release_slots?.[0];return `<tr><td><strong>${UI.esc(t.team_code)}</strong><div class="help">${UI.esc(t.team_name||'-')}</div></td><td>${statusBadge(t.status)}</td><td>${UI.esc(t.payment_status)}</td><td>${slot?UI.dateTimeTH(slot.scheduled_at):'-'}</td><td>${t.total_score}</td><td><div class="row-actions"><button class="btn mini" data-approve-team="${t.id}">${t.status==='approved'?'ยกเลิกอนุมัติ':'อนุมัติ'}</button><button class="btn mini" data-release-team="${t.id}">ปล่อยตัวตอนนี้</button></div></td></tr>`}).join('')}</tbody></table></div>`;
  ...
  async function generateSlots(){
    ...
    const teams=state.teams.filter(t=>!['cancelled','disqualified'].includes(t.status));if(!teams.length){UI.toast('ยังไม่มีทีมสำหรับสร้างเวลา','error');return;}
    const base=new Date(`${ev.event_date}T${ev.race_start_time.slice(0,8)}+07:00`);const rows=teams.map((t,i)=>({event_id:ev.id,team_id:t.id,scheduled_at:new Date(base.getTime()+i*ev.release_interval_minutes*60000).toISOString(),release_status:'scheduled'}));
    const {error}=await db.from('trail_release_slots').upsert(rows,{onConflict:'event_id,team_id'});if(error){UI.toast(error.message,'error');return;}UI.toast(`สร้างเวลาออกตัว ${rows.length} ทีมแล้ว`,'success');await loadTeams();
  }
  async function releaseNow(teamId){
    const now=new Date().toISOString();const team=state.teams.find(t=>t.id===teamId);if(!team)return;
    const {error:e1}=await db.from('trail_teams').update({status:'racing',started_at:now}).eq('id',teamId);if(e1){UI.toast(e1.message,'error');return;}
    const {error:e2}=await db.from('trail_release_slots').upsert({event_id:state.eventId,team_id:teamId,scheduled_at:team.trail_release_slots?.[0]?.scheduled_at||now,actual_released_at:now,release_status:'released'},{onConflict:'event_id,team_id'});if(e2){UI.toast(e2.message,'error');return;}UI.toast(`${team.team_code} ปล่อยตัวแล้ว`,'success');await loadTeams();await loadDashboard();
  }
