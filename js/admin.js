(() => {
  ...
  async function runSystemCheck(){
    ...
    box.innerHTML='<div class="help">กำลังตรวจ Database, Frontend และ miniapp-api...</div>';
    const checks=[];
    ...
    try{
      ...
      if(error) add('miniapp-api',false,UI.toMessage?UI.toMessage(error):error.message);
      else add('miniapp-api',data?.ok===true && data?.version==='10.0.0',`version ${data?.version||'-'}`);
