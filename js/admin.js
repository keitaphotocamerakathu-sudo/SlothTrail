(() => {
  ...
  if (!db || !UI) {
    const missing = [!db ? 'trailDb' : '', !UI ? 'TrailUI' : ''].filter(Boolean).join(', ');
    console.error('Trail RC Admin dependencies missing:', missing);
    const box = document.createElement('div');
    box.style.cssText='position:fixed;inset:16px auto auto 50%;transform:translateX(-50%);z-index:99999;max-width:760px;width:calc(100% - 32px);padding:14px 16px;background:#4b1f2a;color:#fff;border:1px solid #a84d5c;border-radius:12px;font:16px/1.5 system-ui';
    box.textContent='ไฟล์ระบบ Admin โหลดไม่ครบ: '+missing+' — ตรวจว่า js/config.js, js/supabase-client.js และ js/common.js อยู่ใน GitHub';
  ...
  async function duplicateEvent(id){
    ...
    const newSlug=`${slugBase}-${String(Date.now()).slice(-5)}`;
    const {data,error}=await db.rpc('trail_duplicate_event',{
      p_source_event_id:id,
      p_name:copyName.trim(),
      p_slug:newSlug
    });
    if(error){
      const msg=String(error.message||'');
      if(msg.includes('trail_duplicate_event')){
        UI.toast('ฐานข้อมูลยังไม่มี Duplicate Event v10 กรุณารัน v10_finalize.sql ครั้งเดียว','error');
      }else if(msg.includes('ADMIN_REQUIRED')||msg.includes('EVENT_ADMIN_REQUIRED')){
        UI.toast('บัญชีนี้ไม่มีสิทธิ์ Duplicate Event','error');
      }else UI.toast(msg,'error');
      return;
    }
    const newEvent=Array.isArray(data)?data[0]:data;
    if(!newEvent?.id){UI.toast('Duplicate Event ไม่สำเร็จ: ไม่ได้รับ Event ใหม่จากฐานข้อมูล','error');return;}
    UI.toast('Duplicate Event สำเร็จ · คัดลอก Settings, Registration และ RC แล้ว (ไม่คัดลอกทีม/QR)','success');
    state.eventId=newEvent.id;
    await loadEvents();
  }
  ...
  function showQrToken(cp,type,raw){
    ...
    const wrap=document.createElement('div');wrap.id='qrTokenModal';wrap.className='modal-backdrop show';wrap.innerHTML=`<div class="modal" style="max-width:520px"><div class="modal-head"><h3>${type==='real'?'QR จริง':'QR หลอก'} · RC ${cp.checkpoint_no}</h3><button id="qrClose" class="btn btn-ghost mini">ปิด</button></div><div class="notice">ระบบเก็บ Token ฉบับจริงไว้สำหรับเปิด/พิมพ์ QR ซ้ำ และเก็บ SHA-256 hash สำหรับตรวจสอบตอนสแกน กรุณาจำกัดสิทธิ์ Admin และอย่าเผย Token นอกงาน</div><div id="qrCanvas" style="background:white;padding:18px;width:max-content;max-width:100%;margin:18px auto;border-radius:14px"></div><div class="help" style="word-break:break-all"><span class="code">${UI.esc(raw)}</span></div><div class="modal-actions"><button id="copyToken" class="btn">คัดลอก Token</button><button id="printToken" class="btn btn-primary">พิมพ์ QR</button></div></div>`;document.body.appendChild(wrap);
  ...
  async function runSystemCheck(){
    const box=$('systemCheckResult');
    if(!box)return;
    const btn=$('systemCheckBtn');
    if(btn)btn.disabled=true;
    box.innerHTML='<div class="help">กำลังตรวจ Internet, Database, Frontend และ miniapp-api...</div>';
    const checks=[];
    const add=(name,ok,detail='')=>checks.push({name,ok:!!ok,detail:String(detail||'')});

    // Browser cannot read cellular/Wi‑Fi signal bars directly. We check real connectivity,
    // network hints (when supported) and the round-trip time to our Supabase backend instead.
    const conn=navigator.connection||navigator.mozConnection||navigator.webkitConnection||null;
    const netHints=[];
    if(conn?.effectiveType) netHints.push(String(conn.effectiveType).toUpperCase());
    if(Number.isFinite(conn?.downlink)) netHints.push(`~${conn.downlink} Mbps`);
    if(Number.isFinite(conn?.rtt)) netHints.push(`RTT hint ${conn.rtt} ms`);
    if(conn?.saveData) netHints.push('Data Saver');
    add('สถานะ Internet',navigator.onLine,navigator.onLine?(netHints.length?`ออนไลน์ · ${netHints.join(' · ')}`:'ออนไลน์'):'ออฟไลน์');
    ...
    try{
      ...
      if(!url) throw new Error('ไม่พบ SUPABASE_URL');
      const controller=new AbortController();
      const timer=setTimeout(()=>controller.abort(),8000);
      const res=await fetch(`${url}/auth/v1/settings?trail_ping=${Date.now()}`,{
        method:'GET',
        headers:cfg.SUPABASE_PUBLISHABLE_KEY?{apikey:cfg.SUPABASE_PUBLISHABLE_KEY}:{},
        cache:'no-store',
      ...
      const quality=backendLatencyMs<=250?'ดีมาก':backendLatencyMs<=600?'ดี':backendLatencyMs<=1200?'พอใช้':backendLatencyMs<=2500?'ช้า':'ช้ามาก';
      add('Internet → Supabase',true,`${backendLatencyMs} ms · ${quality}`);
    }catch(e){
      add('Internet → Supabase',false,navigator.onLine?`เชื่อม Backend ไม่สำเร็จ · ${e.name==='AbortError'?'timeout > 8 วินาที':(e.message||e)}`:'อุปกรณ์ออฟไลน์');
    }

    try{
      const {data,error}=await db.rpc('trail_system_check');
      if(error){
        add('Database v10',false,error.message||error);
      }else{
        add('Database v10',data?.version==='10.0.0',`version ${data?.version||'-'}`);
        const f=data?.functions||{};
        add('สร้าง Event (trail_create_event)',!!f.trail_create_event);
        add('Duplicate Event (trail_duplicate_event)',!!f.trail_duplicate_event);
        add('ปล่อยตัวทีม (trail_release_team)',!!f.trail_release_team);
        add('สิทธิ์ Admin',data?.admin===true,data?.admin?'พร้อม':'บัญชีนี้ไม่ใช่ Admin');
        const c=data?.columns||{};
        add('GPS ราย RC',!!c.gps_required && !!c.gps_radius_meters);
        add('pgcrypto',data?.pgcrypto===true);
      }
    }catch(e){add('Database v10',false,e.message||e)}
    add('Leaflet Map',!!window.L,window.L?.version?`v${window.L.version}`:'โหลดไม่สำเร็จ');
    add('QR Generator',!!window.QRCode,window.QRCode?'พร้อม':'โหลดไม่สำเร็จ');
    try{
      const edgeStarted=performance.now();
      const {data,error}=await db.functions.invoke('miniapp-api',{body:{action:'health'}});
      const edgeMs=Math.round(performance.now()-edgeStarted);
      if(error) add('miniapp-api',false,`${UI.toMessage?UI.toMessage(error):error.message} · ${edgeMs} ms`);
      else add('miniapp-api',data?.ok===true && data?.version==='10.0.0',`version ${data?.version||'-'} · ${edgeMs} ms`);
    }catch(e){add('miniapp-api',false,e.message||e)}
    const okCount=checks.filter(x=>x.ok).length;
    box.innerHTML=`<div style="display:grid;gap:8px">${checks.map(x=>`<div class="notice" style="display:flex;gap:10px;align-items:flex-start;justify-content:space-between"><div><strong>${UI.esc(x.name)}</strong>${x.detail?`<div class="help">${UI.esc(x.detail)}</div>`:''}</div><span class="badge badge-${x.ok?'green':'red'}">${x.ok?'พร้อม':'ต้องแก้'}</span></div>`).join('')}</div><div class="help" style="margin-top:10px">ผ่าน ${okCount}/${checks.length} รายการ · Trail RC v10</div>`;
